...
cl_program_cache_state cl_program_cache_init(cl_device_id device) {
    ...
    const char * enabled = std::getenv("GGML_OPENCL_KERNEL_CACHE");
    if (enabled && (!std::strcmp(enabled, "0")    || !std::strcmp(enabled, "off")  ||
                    !std::strcmp(enabled, "none") || !std::strcmp(enabled, "disable") ||
                    !std::strcmp(enabled, "disabled"))) {
        if (cache_debug_enabled()) {
            fprintf(stderr, "ggml_opencl: kernel cache disabled by GGML_OPENCL_KERNEL_CACHE=%s\n", enabled);
            fflush(stderr);
        }
        return st;
    }

    const char * env_name = "GGML_OPENCL_KERNEL_CACHE_DIR";
    const char * env = std::getenv(env_name);
    if (!env || !*env) {
        const char * legacy_env = std::getenv("GGML_OPENCL_CACHE_DIR");
        if (legacy_env && *legacy_env) {
            env_name = "GGML_OPENCL_CACHE_DIR";
            env = legacy_env;
        }
    }
    if (env && (!std::strcmp(env, "0")    || !std::strcmp(env, "off")  ||
        ...
        if (cache_debug_enabled()) {
            fprintf(stderr, "ggml_opencl: kernel cache disabled by %s=%s\n", env_name, env);
            fflush(stderr);
        }
        return st;
    }
