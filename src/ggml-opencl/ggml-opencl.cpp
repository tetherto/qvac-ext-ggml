...
struct ggml_backend_opencl_context {
    ...
    cl_kernel kernel_geglu, kernel_reglu, kernel_swiglu, kernel_swiglu_oai, kernel_geglu_erf, kernel_geglu_quick, kernel_siglu,
              kernel_geglu_f16, kernel_reglu_f16, kernel_swiglu_f16, kernel_geglu_erf_f16, kernel_geglu_quick_f16, kernel_siglu_f16;
...
static void load_cl_kernels(ggml_backend_opencl_context *backend_ctx, ggml_cl_version opencl_c_version) {
    ...
    // glu
    {
        ...
        CL_CHECK((backend_ctx->kernel_siglu           = clCreateKernel(backend_ctx->program_glu, "kernel_siglu", &err), err));
        CL_CHECK((backend_ctx->kernel_siglu_f16       = clCreateKernel(backend_ctx->program_glu, "kernel_siglu_f16", &err), err));
        GGML_LOG_CONT(".");
    }
...
static void ggml_cl_glu(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    switch (ggml_get_glu_op(dst)) {
        ...
        case GGML_GLU_OP_SIGLU:
            if (dst->type == GGML_TYPE_F32) {
                kernel = backend_ctx->kernel_siglu;
            } else {
                kernel = backend_ctx->kernel_siglu_f16;
            }
            break;
        default:
