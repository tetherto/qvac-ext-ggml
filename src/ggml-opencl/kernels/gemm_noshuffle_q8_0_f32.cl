...
// F32 twin of kernel_gemm_noshuffle_q8_0_f32 for GGML_PREC_F32 callers. Same tiling and
// indexing; the activations and the accumulators stay float, so activation magnitudes
// past fp16 range (T5-family encoders) neither saturate nor lose their low bits.
#ifdef ADRENO_GPU
REQD_SUBGROUP_SIZE_128
#endif

kernel void kernel_gemm_noshuffle_q8_0_f32_f32acc(
        global const uint * src0_q,
        global const half  * src0_d,
        __read_only image1d_buffer_t src1,
        global float * dst,
    ...
    __global const half* sptr = src0_d + gx_2;
      ...
      for (int i = 0; i < k; i += 4) {
        uint4 pack4 = vload4(0, wptr + (i / 4) * m);
        float4 scale = convert_float4(vload4(0, sptr + (i / 32) * m));
        ...
        float4 wj0 = convert_float4((char4)(p0.s0, p1.s0, p2.s0, p3.s0)) * scale;
        ...
        float4 wj1 = convert_float4((char4)(p0.s1, p1.s1, p2.s1, p3.s1)) * scale;
        ...
        float4 wj2 = convert_float4((char4)(p0.s2, p1.s2, p2.s2, p3.s2)) * scale;
        ...
        float4 wj3 = convert_float4((char4)(p0.s3, p1.s3, p2.s3, p3.s3)) * scale;
