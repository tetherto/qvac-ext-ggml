...
__kernel void flash_attn_f32_f16(
    ...
    ACC_TYPE m_i = -1e30f; // [qvac-21623] Adreno: exp(-inf) returns NaN; use a finite sentinel
    ACC_TYPE l_i = 0.0f;
    ...
    for (int k_start = 0; k_start < n_kv; k_start += BLOCK_N) {
        for (int i = tid; i < BLOCK_N * DK_VEC; i += WG_SIZE) {
            ...
            if (k_row_idx < n_kv) {
                ...
                l_k[row][col] = (KV_DATA_TYPE4)(0.0f); // [qvac-21623] zero-init padding: p*uninit=0*NaN=NaN
            }
        }
        for (int i = tid; i < BLOCK_N * DV_VEC; i += WG_SIZE) {
            ...
            if (v_row_idx < n_kv) {
                ...
                l_v[row][col] = (KV_DATA_TYPE4)(0.0f); // [qvac-21623] zero-init padding (avoid 0*NaN)
            }
        }
        ...
        for (int j = 0; j < BLOCK_N; j += 2) {
            ...
            // [qvac-21623] the mask contains -inf; keep exp() args finite (Adreno NaN-safe).
