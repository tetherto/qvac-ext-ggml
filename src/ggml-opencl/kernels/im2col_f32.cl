...
kernel void kernel_im2col_f32(
    ...
    // up to 1792 bytes in the ACE-Step VAE. Every store then owned a cache line, and
    // im2col measured 81% of the entire VAE decode on an Adreno 740.
    ...
    // integer division in software. Only the final addresses stay 64-bit.
    ...
    for (int col = (int) get_local_id(0); col < CHWi; col += lsz) {
        ...
        dst[offset_row + col] = (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW)
            ? 0.0f
            : src1[offset_bat + (long) iic * delta_offset + iih * IW + iiw];
    }
}
