/**
 * Shared helpers for the qvac-ext-ggml runner-label catalog.
 *
 * Source of truth: .github/runners.yaml
 * Generated consumer: .github/workflows/reusable-runner-names.yml
 *
 * Unlike single-token runner fleets, this repo also uses composite self-hosted
 * label SETS (`[self-hosted, Linux, X64, NVIDIA]`). A catalog entry is therefore
 * either a scalar label or an ordered array of labels; array entries are
 * exported as a JSON-array string and consumed with `fromJSON(...)` in runs-on.
 */
...
// Only these workflows are wired/validated. Orchestration-only files that use
// nothing but ubuntu-latest (release, check-approvals, security-baseline) stay
// out of scope.
const ADDON_WORKFLOWS = new Set(['ci.yml'])
...
export function renderReusableWorkflow(runners) {
  ...
  // Single-quote the RHS in bash so JSON-array double quotes survive verbatim.
 ...
 * composite flow-array form (`runs-on: [self-hosted, Linux, X64, NVIDIA]`).
 * `matrix.os` identities and `if:` conditionals are intentionally not flagged.
 */
export function findHardcodedLabelViolations(relativePath, source, runners) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // `${{ }}` expression: a quoted scalar label inlined instead of via outputs.
    if (value.includes('${{')) {
      const hit = scalars.find((entry) => new RegExp(`['"]${escapeLabel(entry.label)}['"]`).test(value))
